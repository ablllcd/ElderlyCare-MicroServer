...
import com.elderlycare.mapper.UserHealthInfoMapper;
import com.elderlycare.mapper.openfeign.RemoteUserDeviceService;
...
public class UserHealthService {
    ...
    @Autowired
    RemoteUserDeviceService remoteUserDeviceService;

    List<UserHealthInfo> healthInfoCache = new ArrayList<>();
    final int CACHE_THRESHOLD = 100;

    public List<UserHealthInfo> getHistoryHealthInfo(int userID, HistoryHealthInfoRequest request) {
    ...
    public UserHealthInfo getCurrentHealthInfo(int userID) {
        Integer id = userID;

        //先检查用户是否有设备，没有设备返回空数据
        ArrayList<String> devices = remoteUserDeviceService.getDevice(userID);
        if (devices.size() == 0) {
    ...
    public void receiveHealthInfo(InsertHealthInfoRequest infoRequest) {
        // 1. 查找设备匹配的用户
        int userID = remoteUserDeviceService.getUserByDevice(infoRequest.getDeviceID());

        // 2. 存储该用户的健康信息
        UserHealthInfo userHealthInfo = new UserHealthInfo(userID, infoRequest.getTimestamp(), infoRequest.getHeartRate(),
